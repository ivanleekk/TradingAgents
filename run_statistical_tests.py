...
def calculate_tests():
    ...
    if (
        llm_col not in df.columns
        or bench_col_actual not in df.columns
        or bench_col_sim not in df.columns
    ):
    ...
    rf_series = portfolio_common.load_us_3m_tbill_series(
    ...
    # rf_rate_period = portfolio_common.get_period_risk_free_returns(
    #     llm_ret_w.index, 52, None, fallback_rate=0.015
    # )
    ...
    print(
        f"Weekly-based Annualized Sharpe - LLM: {sh_w_llm:.3f}, Bench: {sh_w_bench:.3f}"
    )
    ...
    cov_matrix = np.cov(
        llm_excess_w.loc[common_idx], bench_excess_w.loc[common_idx], ddof=1
    )
