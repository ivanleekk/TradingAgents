...
def compute_bl_weights(
    ...
    try:
        ...
        return weights if weights else {t: mcaps[t]/sum(mcaps.values()) for t in active_set}
...
def run_bl_backtest(
    ...
    for date in dates:
        ...
        weights = compute_bl_weights(active_set, history, market_caps, views, confidences)
...
def main():
    ...
    tickers = ["SPY", "QQQ", "EEM", "TLT", "LQD", "GLD", "USO", "PDBC", "UUP", "FXE", "GBTC", "VNQ", "EWS", "FXI", "EWU", "EWJ", "EMB", "VNQI"]
    ...
    for name, sigs in strategies:
        ...
        curve, wlog = run_bl_backtest(name, sigs, weekly_prices, weekly_exec_prices, daily_prices, market_caps)
        curves[name] = curve
